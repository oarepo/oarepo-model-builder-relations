...
from referrer.proxies import current_service
from referrer.records.api import ReferrerRecord


@click.group(name="referrer")
def group():
    pass
...
def reindex(recreate, queue, *args, **kwargs):
    if recreate:
        index = ReferrerRecord.index._name
    ...
    id_generator = (
        x[0]
        for x in db.session.query(ReferrerRecord.model_cls.id).filter(
            ReferrerRecord.model_cls.is_deleted.is_(False)
        )
    )
    ...
    if queue:
        ...
        for rec_id in tqdm.tqdm(ids, "Indexing records"):
            record = ReferrerRecord.get_record(rec_id)
