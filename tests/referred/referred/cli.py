...
from referred.proxies import current_service
from referred.records.api import ReferredRecord


@click.group(name="referred")
def group():
    pass
...
def reindex(recreate, queue, *args, **kwargs):
    if recreate:
        index = ReferredRecord.index._name
    ...
    id_generator = (
        x[0]
        for x in db.session.query(ReferredRecord.model_cls.id).filter(
            ReferredRecord.model_cls.is_deleted.is_(False)
        )
    )
    ...
    if queue:
        ...
        for rec_id in tqdm.tqdm(ids, "Indexing records"):
            record = ReferredRecord.get_record(rec_id)
